...
class ArticleManager {
    ...
    setupReadingProgress() {
        // Use existing progress bar if available, or create new one
        let progressContainer = document.querySelector('.reading-progress');
        ...
        if (!progressContainer) {
            ...
            progressContainer.innerHTML = `
                ...
                <div class="reading-progress__percentage" id="progressPercentage">0%</div>
            `;
        ...
        window.addEventListener('scroll', () => {
            ...
            if (progressPercentage) {
                progressPercentage.textContent = `${Math.round(remaining)}%`;
...
/* ===============================
   ACTIVE READERS SYSTEM
   Premium UX Feature for Reader Engagement
   =============================== */

BlogApp.prototype.initializeActiveReaders = function() {
    console.log('🔄 Initializing Advanced Active Readers System...');
    
    this.activeReadersConfig = {
        // Base configuration
        minReaders: 33,
        maxReaders: 2300,
        currentReaders: this.getInitialReaderCount(),
        
        // Timing configuration (more realistic)
        updateInterval: 45000, // 45 seconds - más lento y creíble
        microUpdateInterval: 8000, // 8 seconds for micro-adjustments
        ...
        // Trend system
        currentTrend: 'stable', // 'growing', 'declining', 'stable', 'volatile'
        trendDuration: 0,
        trendMaxDuration: 12, // 12 updates = ~9 minutes
        ...
        // Realism factors
        bounceRate: 0.35, // 35% bounce rate (realistic for blogs)
        averageSessionLength: 4.2, // 4.2 minutes average
        peakHours: [11, 14, 20, 21], // Most active hours
        
        element: null
    };
    
    // Find the element
    this.activeReadersConfig.element = document.getElementById('activeReaders');
    
    if (this.activeReadersConfig.element) {
        this.startAdvancedReadersSystem();
        console.log('✅ Advanced Active Readers System initialized successfully');
    } else {
        console.log('ℹ️ Active Readers element not found on this page');
    }
};

BlogApp.prototype.startAdvancedReadersSystem = function() {
    // Set initial realistic value
    this.updateActiveReaders();
    
    // Main update cycle (major changes)
    setInterval(() => {
        this.updateActiveReaders();
    }, this.activeReadersConfig.updateInterval);
    
    // Micro-adjustments (subtle changes)
    setInterval(() => {
        this.microAdjustReaders();
    }, this.activeReadersConfig.microUpdateInterval);
    
    // Track session progression
    setInterval(() => {
        this.trackSessionProgression();
    }, 60000); // Every minute
};

// Generate realistic daily traffic patterns based on web analytics
BlogApp.prototype.generateDailyPattern = function() {
    // Based on real blog analytics: higher traffic during work hours and evening
    return {
        0: 0.15,  // 12 AM - very low
        1: 0.12,  // 1 AM - lowest
        2: 0.10,  // 2 AM - lowest
        3: 0.11,  // 3 AM - very low
        4: 0.13,  // 4 AM - very low
        5: 0.18,  // 5 AM - early risers
        6: 0.25,  // 6 AM - morning start
        7: 0.35,  // 7 AM - commute time
        8: 0.45,  // 8 AM - work start
        9: 0.60,  // 9 AM - work hours
        10: 0.75, // 10 AM - peak morning
        11: 0.85, // 11 AM - high activity
        12: 0.90, // 12 PM - lunch peak
        13: 0.95, // 1 PM - highest daytime
        14: 1.00, // 2 PM - absolute peak
        15: 0.88, // 3 PM - still high
        16: 0.82, // 4 PM - afternoon
        17: 0.75, // 5 PM - end of work
        18: 0.70, // 6 PM - dinner time
        19: 0.80, // 7 PM - evening rise
        20: 0.95, // 8 PM - peak evening
        21: 1.00, // 9 PM - night peak
        22: 0.85, // 10 PM - winding down
        23: 0.45  // 11 PM - late night
    };
};

BlogApp.prototype.generateWeeklyPattern = function() {
    // Based on real data: weekdays higher, weekends different patterns
    const today = new Date().getDay();
    const patterns = {
        0: 0.65, // Sunday - lower
        1: 1.00, // Monday - high
        2: 1.05, // Tuesday - peak
        3: 1.08, // Wednesday - highest
        4: 0.95, // Thursday - good
        5: 0.75, // Friday - dropping
        6: 0.55  // Saturday - weekend low
    };
    return patterns[today];
};

BlogApp.prototype.getInitialReaderCount = function() {
    const now = new Date();
    const hour = now.getHours();
    const dayMultiplier = this.generateWeeklyPattern();
    const hourMultiplier = this.generateDailyPattern()[hour];
    
    // Calculate realistic base for this time
    const baseReaders = 180; // Base average
    return Math.floor(baseReaders * dayMultiplier * hourMultiplier);
};

BlogApp.prototype.updateActiveReaders = function() {
    const config = this.activeReadersConfig;
    const now = new Date();
    const hour = now.getHours();
    
    // Update trend system
    this.updateTrendSystem();
    
    // Get current multipliers
    const dayMultiplier = config.weeklyPattern;
    const hourMultiplier = config.dailyPattern[hour];
    const trendMultiplier = this.getTrendMultiplier();
    
    // Calculate ideal reader count for this moment
    const baseReaders = 180;
    const idealReaders = Math.floor(baseReaders * dayMultiplier * hourMultiplier * trendMultiplier);
    
    // Apply realistic drift toward ideal value
    let targetReaders;
    const driftForce = 0.15; // How strongly it pulls toward ideal
    const randomNoise = 0.25; // Random fluctuation
    
    // Calculate drift toward ideal
    const drift = (idealReaders - config.currentReaders) * driftForce;
    const noise = (Math.random() - 0.5) * config.currentReaders * randomNoise;
    
    targetReaders = config.currentReaders + drift + noise;
    
    // Apply trend-specific behavior
    targetReaders = this.applyTrendBehavior(targetReaders, idealReaders);
    
    // Natural boundaries with organic feel
    targetReaders = Math.max(config.minReaders, Math.min(config.maxReaders, Math.round(targetReaders)));
    
    // Simulate realistic user behavior patterns
    targetReaders = this.simulateUserBehavior(targetReaders);
    
    // Update and animate
    config.currentReaders = targetReaders;
    this.animateReaderCount(targetReaders);
    
    console.log(`📊 Readers: ${targetReaders} | Trend: ${config.currentTrend} | Hour: ${hour} (${(hourMultiplier*100).toFixed(0)}%)`);
};

BlogApp.prototype.updateTrendSystem = function() {
    const config = this.activeReadersConfig;
    
    config.trendDuration++;
    
    // Change trend when duration expires or randomly
    if (config.trendDuration >= config.trendMaxDuration || Math.random() < 0.08) {
        const trends = ['growing', 'declining', 'stable', 'volatile'];
        const weights = [0.35, 0.25, 0.30, 0.10]; // Growing most likely
        
        config.currentTrend = this.weightedRandomChoice(trends, weights);
        config.trendDuration = 0;
        config.trendMaxDuration = Math.floor(Math.random() * 8) + 8; // 8-16 updates
        
        console.log(`📈 Trend changed to: ${config.currentTrend} for ${config.trendMaxDuration} cycles`);
    }
};

BlogApp.prototype.getTrendMultiplier = function() {
    const trend = this.activeReadersConfig.currentTrend;
    const progress = this.activeReadersConfig.trendDuration / this.activeReadersConfig.trendMaxDuration;
    
    switch (trend) {
        case 'growing':
            return 1 + (progress * 0.4); // Up to 40% growth over time
        case 'declining':
            return 1 - (progress * 0.3); // Up to 30% decline
        case 'volatile':
            return 1 + Math.sin(progress * Math.PI * 4) * 0.2; // Oscillating
        default: // stable
            return 1 + (Math.random() - 0.5) * 0.1; // ±5% variation
    }
};

BlogApp.prototype.applyTrendBehavior = function(currentValue, idealValue) {
    const trend = this.activeReadersConfig.currentTrend;
    
    switch (trend) {
        case 'growing':
            // Tendency to be above ideal, with growth spurts
            if (Math.random() < 0.3) {
                currentValue *= 1.1; // 30% chance of 10% boost
            }
            return Math.max(currentValue, idealValue * 0.95);
            
        case 'declining':
            // Tendency toward lower values, with occasional drops
            if (Math.random() < 0.2) {
                currentValue *= 0.85; // 20% chance of 15% drop
            }
            return Math.min(currentValue, idealValue * 1.05);
            
        case 'volatile':
            // Large swings possible
            if (Math.random() < 0.15) {
                currentValue *= Math.random() < 0.5 ? 0.7 : 1.3; // Big swing
            }
            break;
    }
...
BlogApp.prototype.simulateUserBehavior = function(readerCount) {
    const config = this.activeReadersConfig;
    
    // Simulate users joining/leaving based on realistic patterns
    if (Math.random() < 0.4) { // 40% chance of user activity
        const activity = Math.random();
        
        if (activity < 0.15) {
            // Small group joins (viral sharing, social media)
            readerCount += Math.floor(Math.random() * 8) + 3;
        } else if (activity < 0.25) {
            // Some users leave (bounce rate simulation)
            readerCount -= Math.floor(Math.random() * 5) + 1;
        } else if (activity < 0.28) {
            // Sudden interest spike (trending topic)
            readerCount += Math.floor(Math.random() * 15) + 8;
        }
    }
    
    // Ensure natural minimum - never completely empty unless very late
    const now = new Date();
    const hour = now.getHours();
    const naturalMinimum = (hour >= 2 && hour <= 5) ? 
        Math.max(25, config.minReaders * 0.8) : // Very late night can go lower
        config.minReaders;
    
    return Math.max(naturalMinimum, readerCount);
};

BlogApp.prototype.weightedRandomChoice = function(choices, weights) {
...
// Micro-adjustments for subtle realism
BlogApp.prototype.microAdjustReaders = function() {
    const config = this.activeReadersConfig;
    
    // Only make tiny adjustments (±1-3 readers)
    if (Math.random() < 0.6) { // 60% chance of micro-change
        const adjustment = Math.floor(Math.random() * 6) - 3; // -3 to +3
        let newValue = config.currentReaders + adjustment;
        
        // Keep within bounds
        newValue = Math.max(config.minReaders, Math.min(config.maxReaders, newValue));
        
        if (newValue !== config.currentReaders) {
            config.currentReaders = newValue;
            this.updateReaderDisplay(newValue, true); // Micro animation
        }
    }
};

// Track session progression for organic growth
BlogApp.prototype.trackSessionProgression = function() {
    const config = this.activeReadersConfig;
    const sessionMinutes = (Date.now() - config.sessionStartTime) / 60000;
    
    // Simulate organic growth as session continues (word of mouth, sharing)
    if (sessionMinutes > 5) {
        const growthFactor = Math.min(1.2, 1 + (sessionMinutes / 300)); // Cap at 20% growth
        
        // Occasionally add users due to "session effects" (sharing, etc.)
        if (Math.random() < 0.1) { // 10% chance per minute
            const sessionBoost = Math.floor(Math.random() * 4) + 1;
            config.currentReaders = Math.min(config.maxReaders, 
                config.currentReaders + sessionBoost);
        }
    }
    
    // Track reader history for pattern analysis
    config.sessionReaders.push({
        time: Date.now(),
        count: config.currentReaders,
        trend: config.currentTrend
    });
    
    // Keep only last hour of data
    const oneHourAgo = Date.now() - (60 * 60 * 1000);
    config.sessionReaders = config.sessionReaders.filter(record => record.time > oneHourAgo);
};

BlogApp.prototype.updateReaderDisplay = function(targetValue, isMicroUpdate = false) {
    const element = this.activeReadersConfig.element;
    if (!element) return;
    
    if (isMicroUpdate) {
        // Instant update for micro-changes
        const formatted = this.formatReaderCount(targetValue);
        element.textContent = formatted;
    } else {
        // Smooth animation for major changes
        this.animateReaderCount(targetValue);
    }
};

BlogApp.prototype.formatReaderCount = function(count) {
    if (count >= 1000) {
...
BlogApp.prototype.animateReaderCount = function(targetValue) {
    const element = this.activeReadersConfig.element;
    if (!element) return;
    
    const currentDisplayed = parseInt(element.textContent.replace('K', '000').replace('.', '')) || 0;
    const duration = Math.abs(targetValue - currentDisplayed) > 50 ? 3000 : 1500; // Slower for big changes
    const steps = 30; // Smoother, fewer steps
    const stepDuration = duration / steps;
    const valueStep = (targetValue - currentDisplayed) / steps;
    ...
    const animate = () => {
        ...
        const newValue = Math.round(currentDisplayed + (valueStep * steps * easeProgress));
        
        element.textContent = this.formatReaderCount(newValue);
        
        if (step < steps) {
            setTimeout(animate, stepDuration);
        } else {
            // Ensure exact final value
            element.textContent = this.formatReaderCount(targetValue);
        }
    };
